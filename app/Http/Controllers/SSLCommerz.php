...
class SSLCommerz
{
    ...
    {
        $this->print('trx_id',$trx_id);
        $this->print('amount',$amount);
        $this->print('currency',$currency);
        $this->print('post_data',$post_data);
    ...
    protected function validate($merchant_trans_id, $merchant_trans_amount, $merchant_trans_currency, $post_data)
    {
       # MERCHANT SYSTEM INFO
        if ($merchant_trans_id != "" && $merchant_trans_amount != 0) {
            ...
            //$post_data['store_id'] = $this->store_id;
            $post_data['store_pass'] = md5($this->store_pass);
            $this->print('post_data_with_md5 store password',$post_data);
    ...
    protected function SSLCOMMERZ_hash_varify($store_passwd = "", $post_data)
    {
        if (isset($post_data) && isset($post_data['verify_sign']) && isset($post_data['verify_key'])) {
            ...
            $this->print('pre_define_key',$pre_define_key);
            $new_data = array();
            ...
            $this->print('hash_string',$hash_string);
            $this->print('md 5 hash_string',md5($hash_string));
            $this->print('verify_sign',$post_data['verify_sign']);
    ...
    private function print($title, $data)
    {
        //dump($title);
        //dump($data);
        return;
